const CONFIG={
  ...
  multiplier:1.5,
  waste:{
    mixed:{label:"Mixed Waste",unit:"tonne",price:200},
    wood:{label:"Wood",unit:"tonne",price:130},
    soil:{label:"Soil",unit:"tonne",price:80},
    rubble:{label:"Rubble",unit:"tonne",price:80},
    pops:{label:"POPs Waste",unit:"tonne",price:300},
    mattresses:{label:"Mattresses",unit:"each",price:40},
...
function renderDisposalCostSettings(){
  ...
  if(!el)return;
  el.innerHTML=Object.entries(CONFIG.waste).map(([key,w])=>`<label class="cost-setting-row"><span>${w.label} <small>(${w.unit})</small></span><input data-disposal-cost="${key}" type="number" min="0" step="0.01" inputmode="decimal" value="${w.price}"></label>`).join("")+`<label class="cost-setting-row"><span>Labour per load <small>(each load)</small></span><input id="loadLabourSetting" type="number" min="0" step="0.01" inputmode="decimal" value="${getLoadLabour()}"></label><p class="muted">This labour amount is added to every tracked load when calculating the job's actual load cost.</p>`;
}
...
function calculateAiQuote(r){
  const waste={mixed:normaliseAiNumber(r.mixed_tonnes),wood:normaliseAiNumber(r.wood_tonnes),soil:normaliseAiNumber(r.soil_tonnes),rubble:normaliseAiNumber(r.rubble_tonnes),pops:normaliseAiNumber(r.pops_tonnes),mattresses:Math.round(normaliseAiNumber(r.mattresses)),fridges:Math.round(normaliseAiNumber(r.fridges))};
...
function mergeAiEstimates(results){
  const keys=['mixed_tonnes','wood_tonnes','soil_tonnes','rubble_tonnes','pops_tonnes','mattresses','fridges'];
...
function renderAiResult(r){
  const lines=[],labels={mixed:'Mixed waste',wood:'Wood',soil:'Soil',rubble:'Rubble',pops:'POPs waste'};
...
function openNavigation(q){
  const address=(q.address||'').trim();
  if(!address){toast('This job has no address');return}
...
function sendFeedbackWhatsApp(q){
  const b=getBusiness();
  if(!b.facebook){toast('Add your Facebook page link in Owner Settings first');return}
  window.open('https://wa.me/?text='+encodeURIComponent(customerFeedbackMessage(q)),'_blank');
}
function openLoadTracking(i){
  const q=state[i];if(!q)return;
  window.currentLoadJobIndex=i;
  if($("loadJobTitle"))$("loadJobTitle").textContent=`${q.number} · ${q.name||'Customer'}`;
  renderLoadTracking(q);
...
function addLoad(){
  const i=window.currentLoadJobIndex;if(!Number.isInteger(i)||!state[i])return;
  const tonnes=Number($("loadWeight").value),actualRaw=$("loadDisposalCost").value.trim(),actual=actualRaw===''?null:Number(actualRaw);
  if(!Number.isFinite(tonnes)||tonnes<=0){toast('Enter a load weight');return}
  if(actual!==null&&(!Number.isFinite(actual)||actual<0)){toast('Enter a valid disposal cost');return}
...
function renderDashboard(){
  ...
  const visible=state.filter(q=>jobStatus(q)!=='Archived'),earnings=state.filter(q=>jobStatus(q)!=='Denied'&&jobStatus(q)!=='Cancelled');
  const outstanding=visible.filter(q=>!paid(q)).reduce((s,q)=>s+Number(q.quote||0),0);
  const daily=earnings.filter(q=>String(q.date||'').slice(0,10)===day).reduce((s,q)=>s+Number(q.profit||0),0),weekly=earnings.filter(q=>new Date(q.date)>=weekStart).reduce((s,q)=>s+Number(q.profit||0),0),monthly=earnings.filter(q=>{const d=new Date(q.date);return d.getMonth()===month&&d.getFullYear()===now.getFullYear()}).reduce((s,q)=>s+Number(q.profit||0),0);
  const dailyRevenue=earnings.filter(q=>String(q.date||'').slice(0,10)===day).reduce((s,q)=>s+Number(q.quote||0),0),monthlyRevenue=earnings.filter(q=>{const d=new Date(q.date);return d.getMonth()===month&&d.getFullYear()===now.getFullYear()}).reduce((s,q)=>s+Number(q.quote||0),0);
  ...
  if($('dashboardAiLearning'))$('dashboardAiLearning').textContent=getAiLearningStatus();
  const wk=earnings.filter(q=>new Date(q.date)>=weekStart),quoted=wk.reduce((s,q)=>s+Number(q.quote||0),0),paidTotal=wk.filter(paid).reduce((s,q)=>s+Number(q.quote||0),0),out=wk.filter(q=>!paid(q)).reduce((s,q)=>s+Number(q.quote||0),0),profit=wk.reduce((s,q)=>s+Number(q.profit||0),0);
